...
5. 숫자의 범위를 나타낼 때는 반드시 'A ~ B' 형식을 사용하고, 물결표(~) 앞뒤로 공백을 두십시오.
6. 날짜나 수치 사이에 기호가 생략되어 글자가 붙지 않도록 주의하십시오.
7. 반드시 제공된 [데이터]에 있는 정보만을 바탕으로 답변하십시오. 데이터에 없는 내용을 지어내지 마십시오.
8. 어떠한 경우에도 마크다운의 취소선 문법(~~)을 사용하지 마십시오.""",
        ),
        ("user", f"[데이터]:\n{context_combined}\n\n질문:\n{state['question']}"),
    ]
